...
7. User sends the incomplete timestamp file + the public key, and nonces, as well as a signature over the DNS Packet they want to publish to a P2P gossip network (or centralized registries or both).
...
4. The p2p network doesn't need sharding or churning of small nodes like a DHT, because the cost of spamming nodes is as high as generating PoW for a valid Name, and any invalid Name results in blocking the spamming node. Furthermore if the p2p network storage is limited, the user can publish `Retension PoW` which is similar to the `timestamping PoW` but can be generated after timsetamping... and signals commitment to this `Name` to differentiate the user from spammers' `Names`.

## Optimistic self-contained proofs

Especially if we use an optimization resistant hash function for proof of work, we should almost never see a successful second preimage attack, so we should not require a timestamping proof unless we actually know of two different genesises with the same Name.

This could save bandwidth, by not sending the timestamp proof in query responses, but could also be a storage cost saving, by keeping the timestamp proof at the client side, and only publish it to the network when necessary. At the very least nodes can store timestamp proofs in a very cost efficient archives, since they don't need to retrieve the data unless the unlikely event of a successful second preimage attack occurs.

Even more importantly; an optimistic proof, is self contained, and doesn't require checking the Bitcoin network.
